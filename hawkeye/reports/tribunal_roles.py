...
_JUDGE_RULE_JA: dict[int, str] = {
    ...
    3: "**同じ事実から組み立てた空売り側の主張のほうが説得的なら、PASS。**",
    4: "**優位性の正体を名指しできていない(`edge_type=none_identified`)、"
       "または「なぜ今売っている側が間違っているのか」に答えられていない"
       "場合は、PASS。**",
    5: "**確信度(`conviction`)は「気合い」ではなく確率。** 後で答え合わせ"
       "され採点されます。0.55を下回る確信度でBUYと言うのは矛盾なので、"
       "どちらかに寄せる必要があります。",
    6: "**採算のハードル(リワード/リスク比・期待値)はJudgeの後段で"
