...
```bash
hawkeye review-passes --horizon 30 --threshold 15
```

`benchmark` が全体平均を見るのに対し、こちらは**個別銘柄の事後検証**です。
見送った(または承認しなかった)銘柄のうち、その後±15%以上動いたものを
一覧化します。**大きく上昇していた場合、その見送り判断が甘かった可能性が
あります**(逆に下落していれば見送りは妥当だった可能性が高い)。ただし
見送り後に新しい好材料が出ただけの場合もあるため、`hawkeye show <rec_id>`
で当時の反証内容を必ず確認し、「プロセスの欠陥」か「後知恵バイアス」かを
判断してください。
