...
| **決算数値のソース** | 判断の土台になる数値は、企業の1次情報と一致している | **発見=Finnhub / EPS数値=Yahoo に分離済み(2026-08-02)**。ランキングの**前**に第2ソースで取り直し、公表サプライズ率をそのまま採用(再計算しない)。照合上限60件/回、`eps_source` とカレンダー側の読みを両方記録 | **「どちらか一方を正とする」方式そのものを2026-08-02(b)の実測で見直し**、2ソース一致(75%)を答えとし、不一致は企業の提出書類で決着させる設計に変更(**§5.3・未実装**)。売上サプライズはカレンダーのまま(Yahooの決算履歴に売上が無い)。過去に記録済みの候補には遡って適用できない | 実績値の食い違いは25%、うち選別の合否が入れ替わるのは19%。**入れ替わりの2/3はコンセンサス側が原因**で、実績値の精度を上げても解けない。§5.3の実装(a)〜(d)で順に対応 |
| **「良い決算」の定義** | 決算をきっかけに買う以上、その決算が良いことが審査の柱になる | **EPSと売上の2本のみ**。ガイダンス(会社自身が出す次期以降の見通し)は取得も評価もしていない | EPS・売上・ガイダンスの3本で判定する設計を決定(**§5.3・未実装**)。ガイダンスは**ゲートにせず**、有る場合のみ小さな加点、**無いことは減点しない**(出さない会社が多数あるため) | ガイダンスには無料の構造化ソースが存在せず、決算発表文からのLLM抽出が唯一の道。全銘柄に対して読むとトークン費用が見合わないため、**絞り込みの最終段(10〜15件)でのみ読む**設計とした |
| **コンセンサスの取得時点** | 事前予想は「決算前に確定していた予想」である | **決算後に取りに行っている**ため、取得した時点の予想が入る。ソース間の食い違いが「集計方法の違い」なのか「見た時点の違い」なのか区別できない | 決算前日に予想を**事前登録(スナップショット)**する仕組み(**§5.3・未実装**)。これがある場合のみ、売上コンセンサスにも第2のソースが生まれ、アナリスト人数の下限(3人)も判定できる | **時間的に取り返しがつかない項目**。予想のスナップショットは決算前にしか取れず、実行しなかった日のぶんは永久に復元できないため、§5.3の実装順序で最優先に置いた |
| **銘柄マスタ(記録の構造)** | 1銘柄について「過去に何回検討し、決算がどうだったか、いま保有しているか」を追える | **銘柄を表すマスタが無い**。銘柄コードが `recommendations` / `scans` / `screened_candidates` / `drop_reviews` に素の文字列として重複しており、履歴を辿るにも「保有中か・前回いつ審査したか」を知るにも全判断記録を走査するしかない | `stocks`(銘柄マスタ)・決算実績・コンセンサス予想の3テーブルを追加(**§5.3、構造は §6.1・未実装**)。**主キーは銘柄コードではなくSEC登録番号(CIK)**。**最後に審査した会計四半期(`2026-Q2` 形式)**は**台帳から再計算できる投影**として持つ。**保有は1対nなのでマスタに置かず** `positions`(§5.2(2)・未実装)側の担当とし、株価などの時点観測値も置かない | 銘柄コードは上場廃止後に再利用され、社名変更でも変わるため、これを鍵にすると**別会社の履歴が静かに1本に混ざり**、2026-07-28に直した生存者バイアスの分析が再び壊れる |
...
### 5.3 「良い決算」の判定と数値ソースの設計(2026-08-02 設計メモ・実装前)
...
起きていることが分かりました。本節は2026-08-02の実測に基づく設計で、**すべて未実装**です。
承認後に着手します。
...
### 6.1 銘柄マスタと決算データ(2026-08-02 決定・未実装、設計は §5.3)
