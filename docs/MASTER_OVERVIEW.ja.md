...
| **市場データの入手経路** | 判断に使う株価・出来高は、提供元から取得した検証可能な値である | **オフライン投入経路を追加(2026-08-05)**。`HAWKEYE_OFFLINE_DATA` が指す JSON から日足・プロファイルを読み込む(§4参照)。ゲート論理は不変。ファイルに無い項目は `unverified` のままで硬性ゲートは閉じる。投入ファイルの SHA-256 を `notes` に刻み出所を追跡可能にする | **コホート分離が未実装**。手入力データで作られた記録が、Phase 0 の有効性判定(BUY群 vs 見送り群)の統計に混入しうる。また投入ファイルは `var/` 配下で追跡外のため、ハッシュは照合専用で復元はできない | 分離自体は `catalyst.source` の印で事後フィルタ可能だが、`benchmark` 側の明示的除外は未着手。手入力実行の記録が実際に貯まってから、除外を既定にするか判断するのが正しい順序 |
