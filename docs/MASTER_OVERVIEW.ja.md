...
AAPLの行に**前四半期のEPS**($1.91)を載せており(正しくは$2.02)、BJRIについては
...
### 市場データのオフライン投入経路(2026-08-05 追加)

候補情報の組み立て(`build_brief`)は常に実データ提供元へ問い合わせ、日足が空なら
市場スナップショットの構築(`build_snapshot`)が例外で停止します。`case open` の
`--price` 等の引数は**取得値の上書き**であって代替ではないため、提供元に到達
できない環境(組織のegressポリシーによる遮断、APIキー無し)では**候補を1件も
評価できません**でした。

これを解消するため、環境変数 `HAWKEYE_OFFLINE_DATA` で指定したJSONから日足と
プロファイルを読み込む経路(`hawkeye/marketdata/offline.py`)を追加しました。
既存のオフライン用プロバイダ(`StaticProvider`)をCLIから選択できるようにした
だけで、**ゲートの論理は一切変更していません**。

この経路が持つ危険は明白です——**人間が任意の数値を打ち込め、それが台帳の中では
取得値と見分けがつかなくなる**こと。封じ込めとして次の3点を実装しています。

1. **既定にしない**。環境変数が明示的に設定された時のみ有効化し、実行のたびに
   標準エラーへ警告を出す。
2. **ゲートを買えない**。ファイルに無い項目は `None` のままで、従来どおり
   `unverified` として扱われ、硬性ゲートは不合格側に閉じる(invariant 6)。
   実際にSPCXの検証では、決算反応日の日足が欠けている状態で
   イベント時ギャップと経過日数が `None` になり、鮮度ゲートが正しく閉じました。
3. **出所を記録に刻む**。投入ファイルのSHA-256を候補資料の `notes` に書き込む
   (`provenance_note`)。後日この記録を読む人が「この数値は手入力である」と
   判別でき、手元のファイルが記録を生んだものと同一かを照合できる。

**残る限界**(§5のギャップ表に記載): ハッシュは*照合*にしか使えず*復元*は
できません。投入ファイルは `var/` 配下=git管理外にあるため、実行環境が失われれば
ファイル自体は消え、台帳には照合先の無いハッシュだけが残ります。これは
2026-08-01に `cases/` が消失した件と同じ構造の弱点です。
...
| **決算数値のソース** | 判断の土台になる数値は、企業の1次情報と一致している | **発見=Finnhub / EPS数値=Yahoo に分離済み(2026-08-02)**。ランキングの**前**に第2ソースで取り直し、公表サプライズ率をそのまま採用(再計算しない)。照合上限60件/回、`eps_source` とカレンダー側の読みを両方記録 | **売上サプライズはカレンダーのまま**(Yahooの決算履歴に売上が無い)。ガイダンスは依然として無料の構造化ソースが存在しない。過去に記録済みの候補には遡って適用できない | 売上コンセンサスは報告済み四半期については無料で取得する手段が見つかっていない。ガイダンスは決算資料本文のLLM抽出が現実的な唯一の道で、Phase 1以降 |
| **市場データの入手経路** | 判断に使う株価・出来高は、提供元から取得した検証可能な値である | **オフライン投入経路を追加(2026-08-05)**。`HAWKEYE_OFFLINE_DATA` が指す JSON から日足・プロファイルを読み込む(§4参照)。ゲート論理は不変。ファイルに無い項目は `unverified` のままで硬性ゲートは閉じる。投入ファイルの SHA-256 を `notes` に刻み出所を追跡可能にする | **コホート分離が未実装**。オフライン実行は**構造的にPASSへ偏る** — 2026-08-05のSPCX実行では反対役の深刻度5の攻撃が「この記録の数値はすべて未検証の手入力である」であり、裁定役はこれを決定的として採用した。この攻撃は候補の中身に関わらず**あらゆるオフライン実行で成立する**ため、オフライン実行の結果を候補の良し悪しの証拠として読むことはできず、Phase 0のキル基準が依存するBUY群 vs 見送り群の比較から除外しなければならない。また投入ファイルは `var/` 配下で追跡外のため、ハッシュは照合専用で復元はできない | 分離自体は `catalyst.source` の印で事後フィルタ可能だが、`benchmark` 側の明示的除外は**未実装**。オフライン実行が常態化する前に着手すべき項目 |
