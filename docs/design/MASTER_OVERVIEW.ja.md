...
| **「良い決算」の定義** | 決算をきっかけに買う以上、その決算が良いことが審査の柱になる | **EPS・売上・ガイダンスの3本柱で判定済み(2026-08-02実装 / 2026-08-08改訂、`hawkeye/scout/quality.py`)**。上振れは**1つの提供元の実績が、同じ提供元のコンセンサスを上回ったとき**。ガイダンスは**ゲートにせず**、上振れ時のみ小さな加点、**不在は0点で減点なし**。EPSレンジを出さない会社(AMZN等)は**売上レンジの中央値**で判定 | **翌四半期ガイダンスの供給は実装済み(2026-08-08)** — 数値を取るのと同じ応答(決算専門サイトのサマリ文)から決定論的に読む。**通期の物差しは欄だけあって中身が無い**(タスク8.7)。実測では47銘柄中10社が通期のみで見通しを出しており、この10社は今も「ガイダンス開示なし」と誤読される。公表Non-GAAP EPSを読む経路は**2026-08-07に廃止**(発表文の読み取り自体を撤去したため) | 翌四半期ぶんは定型文なのでコードで読める(層1)。**期間の判定と但し書きの解釈だけがエージェントの仕事**(層2・層3)で、そこはトークン費用が要るため上位15枠を確定させた後に走らせる設計。実装はタスク8.7 |
