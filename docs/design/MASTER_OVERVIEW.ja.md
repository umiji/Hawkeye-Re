...
システム内部のドキュメント(`docs/design/ARCHITECTURE.md` など)は英語で書く方針ですが、
...
| 候補発掘(スカウト) | 決算サプライズを機械的にスキャンし、入口ゲートで絞り込む(`hawkeye scout`)。点数計算・並べ替え・台帳記録は行わない — ガイダンス(会社自身の見通し)を読み終えるまで待つ必要があるため、別工程に分離済み(2026-08-12、下記「順位付け」参照) | `hawkeye/scout/scout.py::run_scout`, `earnings.py` |
| 順位付け(2026-08-12、決算3本柱がそろってから) | ガイダンスの読み取り待ち行列が空になった後に`hawkeye rank`で実行。決算3本柱(EPS・売上・ガイダンス)がそろった状態で点数を計算し直し、並べ替え、台帳に記録する。中間ファイル(`var/scan/pending.json`)に走査結果を保存し、記録は一度きり(不変条件1) | `hawkeye/scout/scout.py::rerank_after_guidance`, `hawkeye/scout/scan_store.py` |
...
| コンセンサスの事前登録(2026-08-02 / 2026-08-09に提供元を変更) | 翌2営業日以内に決算予定の銘柄について、決算専門サイト(EW)の決算**前**の窓口(`/api/getstocksdata/`)が返すEPS・売上のコンセンサスと、Finnhubの点推定を、決算**前**に記録(`hawkeye consensus capture`)。値が動いていなければ行を作らない。**実績と同じ提供元にするための変更**——サプライズ率の分子と分母が別の提供元だと率に意味がない。失ったのはアナリスト人数と予想レンジで、どちらも「使わない」と決定済み | `hawkeye/scout/prereg.py`, `hawkeye/marketdata/whispers.py::WhispersSource.forecast` |
| 決算の3本柱判定(2026-08-02実装 / 2026-08-08改訂) | EPS・売上・ガイダンスの3本で判定。**すべての率は「1つの提供元の実績 ÷ 同じ提供元のコンセンサス」**で、提供元は1決算につき1つに決まる。アナリスト不足・分母過小・実績や物差しの欠落は未検証(0点)。もう一方の提供元の実績は記録に残り、差があれば `vendors_report_different_actuals` として審理に届く(判定はしない) | `hawkeye/scout/quality.py`, `hawkeye/scout/numbers.py`, `hawkeye/reports/quality_ja.py` |
| 会社の見通しは1決算につき**複数期**(2026-08-19、T-020) | 1本の発表文が翌四半期と通期の両方に見通しを述べるのは普通のこと(実測: 会社の見通しを述べた21件のサマリーのうち5件、23.8%)。決算行は読み取れた**全期**を保持し、受け付けなかった期はその理由を名前で残す。採点は**一番悪い期だけが支配**する(User決定 2026-08-19)——1つの期の中の判定規則は従来どおり(EPSと売上が混在なら「ほぼ一致」)、その上で期をまたいで最も悪い期を採る。上振れの期を足しても点は上がらないので、有利な期を選べる余地が構造的に無い。全期は画面にも審理の材料にも出る。**これが無かった間、同じ発表文から読み手がどちらの文を選ぶかだけでガイダンスの点が ±5.0 に振れていた**(AME実測) | `hawkeye/contracts/stocks.py::EarningsPrint.guidance_readings`, `hawkeye/scout/quality.py::_guidance_leg`, `hawkeye/scout/guidance_agent.py` |
| 走査CSVで会社の見通しを検算できる(2026-08-19、T-018) | 走査の記録(`screened_candidates`)が、ガイダンスの点数を計算した**元の2つの数字**——会社が示した見通しの中央値と、比べたコンセンサス——を、**期ごと・単位(EPS/売上)ごとに**保持する。走査CSVはそれを6列で出す(対象期／会社見通し／コンセンサス をEPSと売上の2組)。**レポート生成時に台帳から引き直すのではなく走査時に焼き付ける**——決算行は後から見通しを得たり差し替わったりするので、引き直すと「順位付けが実際に使った数字」と食い違い、検算にならない(EPS実績値を走査時に保存しているのと同じ理由)。併せて、CSVのエラー列を「数値」「見通し」「発表文」の3語から1文の説明文に変え、会社がEPSレンジを開示していても「EPSレンジの開示が無いため」と出ていた比較の但し書きを直した | `hawkeye/contracts/models.py::GuidanceComparison`, `hawkeye/scout/quality.py::guidance_comparisons`, `hawkeye/reports/scan_report_ja.py`, `hawkeye/reports/quality_ja.py` |
| 決算行の出所と改訂(2026-08-07) | 決算実績の行は**どのデータ提供元の数値か**(`source`)と**その行がまだ有効か**(`status`)を持つ。実績が後から書き換わった場合は、新しい行を `active` で追記し、古い行を `superseded` に落として**残す**(順位を付けた時点でどの値を見ていたかが消えないため)。SQLiteの一意インデックスは `active` の行だけを対象にした部分インデックスで、トリガーが許す更新は `active`→`superseded` の一方向のみ | `hawkeye/contracts/stocks.py::PrintSource,RowStatus`, `hawkeye/ledger/stocks.py::revise_print` |
| 四半期履歴の穴埋め(2026-08-03) | 選別を通った銘柄と、既にマスタにある銘柄は、その後どこで落ちても四半期の行を残す(§6.1(C))。追加のAPI呼び出しはゼロ。既に行がある四半期は上書きしない | `hawkeye/scout/scout.py::_record_cheap_history` |
| 銘柄を指定した決算審査(2026-08-03) | `hawkeye case open TICKER --from-earnings` — 人が名指しした銘柄について、**発見用の5%選別を通さずに**直近の決算を3本柱で判定し、その読みをそのまま審理の争点(catalyst)にする。決算専門サイトの読み取り・1銘柄1ソースの規則・コンセンサスの固定・四半期の記録はファネルと同一の経路 | `hawkeye/scout/single.py` |
| CLI | 上記すべてをつなぐコマンド群 | `hawkeye/cli.py` |
| セッション駆動 | Claude Codeセッションが3エージェントをオーケストレーションする手順書 | `.claude/skills/hawkeye-run/SKILL.md` |

**【2026-08-07 撤去済み】決算発表文の読み取りと、EDGARのXBRL照合を、走査から
全部外しました。** 会社自身の決算発表文(プレスリリース)を読んで数値を確定させる
処理と、その読み取りが正しいかSECへの提出データと突き合わせる処理は、どちらも
存在しません。理由は1つで、**発火条件が構造的に立たなくなった**からです。発表文を
読みに行く条件は「2つのデータ提供元が違うEPS実績を返したとき」だけでしたが、
EarningsWhispersへの移行で**1つの決算につきデータ提供元は1つ**になるため、
この条件は二度と成立しません。発火しないゲートは、無いゲートより悪い状態です
——安全装置があるように読めてしまうからです。XBRL照合はその読み取りを検算する
ためだけに存在していたので、検算する対象が消えた時点で一緒に外しました。

一緒に消えたもの: 依頼の記録(`release_requests` テーブル)、重複排除の免除、
走査期間外の名指し問い合わせ、`var/releases/` の受け渡し場所、記録の深さ
(`depth`)。**`EdgarDirectory`(銘柄コード→SEC登録番号の対応表)は別物なので
残しています** — 上場廃止後に再利用された銘柄コードで2社の履歴が混ざるのを
防ぐ土台です。

**受け入れた代償**: この系から一次資料による検算が完全に消えました。EW移行後は
EarningsWhispersの数字を正とします(User決定)。もともとXBRLはGAAP基準の提出値、
街のコンセンサスは調整後基準が基本なので、サプライズ率の検算器としては誤報が
多く、銀行・保険・REITなど26%にはXBRL自体が存在しませんでした。副作用として、
「この上振れはGAAP基準で一時要因が混ざっているかもしれない」という警告
(`unadjusted`)が日本語レポートから消えます。この判定は発表文を読まないと
できないためです。「データ提供元自身が矛盾する実績値を返している」という警告
(AMZNの+194%を使用不能にした本体)は残ります。

以下は撤去前の経緯です。**再提案しないための記録**として残します
(`docs/knowledge/REJECTED.ja.md` も参照)。発表文の読み取りは
このプロセスの外(担当者またはエージェント)で行うため、走査は「読んでほしい決算の
名前」を出し、次回の走査が `var/releases/` に置かれた抽出結果を拾う——という2パス
構成でした。2026-08-03(b) 時点で**2回目が成立していない**バグを直しています:
すでに一度でも記録した決算は(ticker, 決算日)で重複排除される仕組み
(`Ledger.seen_events()`)があり、実績値が食い違って未検証のまま落ちた銘柄は
まさに「見逃し候補として記録された銘柄」なので、次回の走査から除外されていました。
**書類は用意されるが誰も読まない**状態です。修正では依頼を台帳に記録し、その1件に
限り重複排除を免除したうえで、走査期間外ならカレンダーに名指しで問い合わせ直して
いました。この教訓自体は生きています——**48時間の実績待ちでも、保留中の決算を重複排除が
弾かないようにする例外が同じ理由で必要**でした。**【2026-08-08 実装済み】**
`Ledger.seen_events()` が `stage="actual_pending"` の行だけを既出から除きます
(打ち切り済みの `actual_timeout` は本物の決着なので通常どおり弾きます)。
...
> ⚠️ **【2026-08-08 更新】この節は当時の設計判断の記録です。結論(発見と数値を
> 分ける)は生きていますが、数値側の提供元と規則が変わりました。** 本文は
> 「なぜそう決めたか」の記録として残し、読み替え表を先に置きます。
>
> | この節の記述 | 現在(2026-08-08) |
> | --- | --- |
> | 数値はYahoo(`yahoo_earnings.py`) | **決算専門サイト EarningsWhispers**(`hawkeye/marketdata/whispers.py`)。`yahoo_earnings.py` は削除 |
> | 移すのは**EPSだけ**、売上はカレンダーのまま | **EPS・売上とも、実績もコンセンサスも同じ提供元から取る。** 片方でも欠ければ**その決算まるごと**カレンダーに戻す |
> | 照合は `hawkeye/scout/verify.py` | `hawkeye/scout/numbers.py`(`verify.py` は削除) |
> | 上限60件(`scout_max_verify`)、Yahooは62回/分 | **50件**(`scout_max_whispers`)、決算専門サイトは**184回/分**で絞りなし |
> | 記録は `eps_source`(`calendar` / `yahoo`) | `numbers_source`(`calendar` / `whispers`) |
> | 「2ソースが一致して初めて上振れ」 | **撤去。** 比べていた2つの数字は同じものを測っていなかった(調整後の予想 vs GAAPの実績)ので、小さい方を選んでも厳しい読みにはならなかった |
> | テストは `tests/test_earnings_verification.py` | `tests/test_earnings_numbers.py` |
> | yfinanceの `lxml` 依存 | 決算数値の経路にyfinanceは無い。事前登録だけが今もyfinance |
>
> **公表サプライズ率をそのまま使う規則は生きています**が、条件が1つ増えました:
> 決算専門サイトは**独自の予想(Whisper値)を基準にした率**も返してくるので、
> 物差しが `consensus` のときだけ採用し、`whisper` のときは破棄して計算し直します
> (AMDは公表値 −1.78%、コンセンサス比 +2.47% で判定が逆転する)。
...
|---|---|---|---|---|
| 1 | サプライズ選別 | 決算実績が事前予想をどれだけ上回ったかで足切り(手元の計算のみ、外部問い合わせなし) | 数百件 | 件数のみ(母集団が大きすぎるため意図的) |
| 1.5 | 実績待ちの保留 | 決算専門サイトがまだ当期の数値を返さない/接続できない決算は順位に載せず保留 | 実行ごとに変動 | ✅ `actual_pending` として**1走査1行**。48時間で `actual_timeout` |
| 2 | 肉付け上限 | 株価等の実データ取得は1銘柄ずつ外部APIを叩くため、スコア順に歩いて打ち切る | 実行ごとに変動 | ✅ `enrichment_cap` として**全項目**(銘柄・スコア・価格・その時見えていたニュース) |
| 3 | 入口ゲート | 流動性・時価総額・カタリストの鮮度などで機械的に足切り | 実行ごとに変動 | ✅ `gate_reject` として全項目＋**ゲートの実測値** |
| 4 | ランキング下位 | ゲートは通ったが、スコア上位N件(通常3件)しか審理に送らない | 実行ごとに変動 | ✅ `ranking_cutoff` として全項目＋順位 |
...
| **決算数値のソース** | 判断の土台になる数値は、企業の1次情報と一致している | **発見=Finnhub / 数値=決算専門サイト(EarningsWhispers)に分離済み(2026-08-08)**。ランキングの**前**に取得し、**15枠の順位はこの数値で決まる**。上限50件/回(`scout_max_whispers`)、`numbers_source` とカレンダー側の読みを両方記録。**一次資料(EDGAR XBRL)による検算は2026-08-07に廃止** | **1決算1提供元を実装済み(2026-08-08)**。実績とコンセンサスは必ず同じ提供元から取る。決算専門サイトが当期のレコードとEPS・売上の両コンセンサスを返したときだけそちらを使い、欠ければ**その決算まるごと**カレンダーに戻す。Finnhubが同じ決算に矛盾する実績を返した場合、その実績は**使用不能**。両提供元の実績が食い違えば `vendors_report_different_actuals` として**報告のみ**行う(判定はしない)。過去に記録済みの候補には遡って適用できない | 実績値の食い違いは25%、うち選別の合否が入れ替わるのは19%。**入れ替わりの2/3はコンセンサス側が原因**で、実績値の精度を上げても解けない。→ 1銘柄1ソースにしたので食い違い自体が判定に入らなくなった。**残る未着手はWhisper加点(タスク8)** |
| **「良い決算」の定義** | 決算をきっかけに買う以上、その決算が良いことが審査の柱になる | **EPS・売上・ガイダンスの3本柱で判定済み(2026-08-02実装 / 2026-08-08改訂、`hawkeye/scout/quality.py`)**。上振れは**1つの提供元の実績が、同じ提供元のコンセンサスを上回ったとき**。ガイダンスは**ゲートにせず**、上振れ時に加点(+5.0)・**下振れ時に減点(-5.0。2026-08-11 User判断で追加)**、**不在は0点で減点なし**。EPSレンジを出さない会社(AMZN等)は**売上レンジの中央値**で判定 | **翌四半期ガイダンスの供給は実装済み(2026-08-08)** — 数値を取るのと同じ応答(決算専門サイトのサマリ文)から決定論的に読む。**通期の物差しは欄だけあって中身が無い**(タスク8.7)。実測では47銘柄中10社が通期のみで見通しを出しており、この10社は今も「ガイダンス開示なし」と誤読される。公表Non-GAAP EPSを読む経路は**2026-08-07に廃止**(発表文の読み取り自体を撤去したため) | 翌四半期ぶんは定型文なのでコードで読める(層1)。**期間の判定と但し書きの解釈だけがエージェントの仕事**(層2・層3)。実装はタスク8.7。**上位15枠を確定させた後にガイダンスを読む設計は2026-08-12に撤回**——セッション方式ではガイダンスを読み終える前に順位が決まってしまい、方式によって選ばれる銘柄が変わる欠陥だった(`docs/design/RANK_AFTER_GUIDANCE.ja.md`)。現在は `hawkeye scout`(ゲート通過まで)→ガイダンス読み取り→`hawkeye rank`(3本柱がそろってから点数計算・順位確定・台帳記録)の順 |
| **コンセンサスの取得時点** | 事前予想は「決算前に確定していた予想」である | **事前登録の仕組みを実装済み(2026-08-02、`hawkeye consensus capture`)**。翌2営業日以内に決算予定の銘柄について、Yahooの平均・上下限・アナリスト人数とFinnhubの点推定を1行に記録。**最新行と同じ値なら行を作らない**ので、2行目は予想が実際に動いたときにだけ生まれる。決算実績を記録した四半期は取得を止める | **運用が要件**: 実行しなかった日の予想は永久に復元できない。事前登録が無い決算は、審査時に**事後復元**として記録され(`snapshot_kind=reconstructed`)、売上は決算専門サイトのコンセンサスで測る(2026-08-08以降。以前は「ソースが1つだけ=未検証」として落ちていた) | **時間的に取り返しがつかない項目**。予想のスナップショットは決算前にしか取れず、実行しなかった日のぶんは永久に復元できないため、§5.3の実装順序で最優先に置いた |
| **銘柄マスタ(記録の構造)** | 1銘柄について「過去に何回検討し、決算がどうだったか、いま保有しているか」を追える | **3テーブルを追加済み(2026-08-02、`hawkeye/ledger/stocks.py`)**。`hawkeye stocks show <銘柄>` で決算履歴・確定コンセンサス・過去の判断が1回の読み出しで出る。`consensus_snapshots` と `earnings_prints` は**SQLiteのトリガーでUPDATE/DELETEを拒否**(APIに更新メソッドが無いだけでは慣習に過ぎないため)。1つの四半期に有効な決算行は**常に1行**で、実績が改訂されたときだけ行が増える(新しい行を追加し、古い行を無効化して残す。2026-08-07変更) | 既存テーブルへの `stock_id` 後埋めは**未実施**(Userの指示により既存記録は書き換えない)。したがって過去の判断との突き合わせは現在も銘柄コード経由で、上場廃止後に再利用された銘柄コードは区別できない。**主キーは銘柄コードではなくSEC登録番号(CIK)**。**最後に審査した会計四半期(`2026-Q2` 形式)**は**台帳から再計算できる投影**として持つ。**保有は1対nなのでマスタに置かず** `positions`(§5.2(2)・未実装)側の担当とし、株価などの時点観測値も置かない | 銘柄コードは上場廃止後に再利用され、社名変更でも変わるため、これを鍵にすると**別会社の履歴が静かに1本に混ざり**、2026-07-28に直した生存者バイアスの分析が再び壊れる |
| **審理枠の繰り上がり** | 機械的に失格でない候補から順に、埋められる限りの審理枠が埋まる | **実装済み(2026-08-02)**。上位から1件ずつ肉付け＋ゲート判定を回し、**ゲートを通った候補が15件**(`scout_target_gate_passed`)貯まるまで続ける。上位15件の固定スライスではないので、14件が落ちれば16位以下が繰り上がる。試行そのものにも天井があり、45件(`scout_max_enrich`)試して埋まらなければ「今日は悪い日」として打ち切る | 差分なし | — |
...
決算カレンダー全件
  → EPSサプライズ 5%以上で足切り
  → スコア順に1件ずつ 詳細取得 → 入口ゲート        ← ここ
  → ゲート通過が15件貯まるまで歩く(試行の天井は45件)
  → スコア順に並べて審理へ
```

> **【2026-08-02 / 2026-08-08 修正】この節は「上位15件の固定スライス」を前提に
> 書かれていますが、**その実装はもうありません**。2026-08-02に「ゲートを通った
> ものを15件集める」方式(`scout_target_gate_passed`=15 / `scout_max_enrich`=45)に
> 変わり、2026-08-08に並び順そのものが決算専門サイトの数値に変わりました。
> **下に述べる逆選択の指摘は依然として有効です** — 詳細取得を始める時点で
> ニュースも株価もまだ取っておらず、順位はサプライズ由来のスコアだけで
> 決まっているためです。

詳細取得を始める時点で**ニュースも株価も一切取得していません**。サプライズ由来の
スコアだけで決まります。
...
採用群を上回っているか」という形で実測できます。**`scout_max_enrich`(45)・
`scout_target_gate_passed`(15)とスコア順という設定は、データが貯まるまで動かしません。**

> ⚠️ **【2026-08-09 撤回】直前の版に「2026-08-08のライブ実行でこの逆選択が実際に
> 観測された（45件の試行が全部 `min_market_cap` で落ちてゲート通過0件）」と書き
> ましたが、**誤りです。観測されていません。**
>
> 原因は検証スクリプトの側でした。肉付け（株価・時価総額の取得）に渡す提供元を
> `YahooProvider` 単体にしていましたが、**Yahooは時価総額を返しません**
> （`profile()` が空の辞書を返す）。時価総額が取れないと、硬い関門は
> 「未検証＝不合格」として扱われます（不変条件6・フェイルクローズ）。つまり
> **全銘柄が「時価総額が小さい」ではなく「時価総額が分からない」で落ちていた**
> だけです。本番（`cli.py`）は `CompositeProvider(YahooProvider(), FinnhubProvider())`
> を渡しており、時価総額はFinnhubから正しく取れています。
>
> **同じ窓を本番と同じ提供元で再実行した結果（2026-08-09）**:
> `決算イベント601件 → 選別157件 → 肉付け24件 → **ゲート通過15件**`
> ——**15枠は埋まりました。** ゲート落ちは9件で、内訳は
> 売買代金6件・売買代金＋時価総額2件・株価1件。
>
> **この節の逆選択の指摘そのものは有効です**（順位は依然としてサプライズ由来の
> スコアだけで決まり、その時点でニュースも株価も見ていない）。ただし
> **「実際に観測された」という記述は取り消します。**
...
`docs/design/USER_GUIDE.ja.md` の構成を「**人が覚えるのはこれだけ**」から始まる形に改め、
...
6. **落選レビュー**((3)(4)) — `drop_reviews` テーブル、抽出処理、Agent向けパッケージ
   の書き出しと検証付き提出、集計
7. **`docs/design/USER_GUIDE.ja.md` の構成変更**((8))
...
> ⚠️ **【2026-08-07 部分的に撤回 / 2026-08-08 追記】この節は当時の設計判断の記録で
> あり、現在の実装ではありません。** 以下は**もう存在しない**ので、読むときに
> 読み替えてください。
>
> ⚠️ **2ソース照合そのものも 2026-08-08 に撤去しました(凍結ではありません)。**
> この節が「保守的」と呼んでいた読み — 小さい方の実績×大きい方の予想 — は、
> **2つの数字が同じものを測っていて初めて保守的**になります。実際には決算専門
> サイトのコンセンサスは調整後ベース、カレンダーの実績はGAAPかもしれない、という
> 別々の物差しでした。したがって小さい方を選ぶことは厳しい読みではなく、
> **意味の無い数字を2つ作って小さい方を選ぶ**ことでした。いまは1決算1提供元で、
> 実績もコンセンサスも同じところから取ります。設定 `earnings_actual_dispute_blocks`
> / `earnings_single_source_consensus_blocks` / `earnings_consensus_dispute_pct` は
> 削除。フラグ `actual_disputed` / `consensus_disputed` / `single_source_actual` /
> `single_source_consensus` / `sources_disagree_on_direction` も削除しました。
> もう一方の提供元の実績は記録に残り、材料差があれば
> `vendors_report_different_actuals` として反論役に届きます(**判定はしない**)。
>
> | この節の記述 | 現在 |
> | --- | --- |
> | 決算発表文を読んで実績を決着させる((4)(5)、実装順序(c)) | **撤去。** 発火条件「2ソースの実績値が食い違う」が、1銘柄1ソースでは構造的に成立しない |
> | EDGARのXBRLで検算する((5)の歯止め1、`eps_xbrl_diluted`) | **撤去。** 検算する対象(発表文の読み取り)が消えたため |
> | 売上実績をXBRLから取る((4)の②) | **撤去。** 売上実績はデータ提供元の値をそのまま使う |
> | Non-GAAP EPS を読む・`unadjusted` の印を付ける((4)の①) | **撤去。** 会社の調整後EPSを読む経路が無くなったため。日本語レポートの「未調整」警告も消えた |
> | 記録の深さ `depth` 4段階 | **廃止。** 「どのデータ提供元の数値か」(`source`)に一本化 |
> | `scout_max_release_reads`(発表文を読む上限3件) | **削除** |
>
> 撤回の理由と受け入れた代償は §4 の【2026-08-07 撤去済み】に、実装タスクの全体像は
> `docs/backlog/PIPELINE_BUILD_TASKS.ja.md` にあります。
> **この節を根拠に発表文読み取り/XBRL照合を再提案しないでください**
> (`docs/knowledge/REJECTED.ja.md`)。
...
>
> **上振れと下振れは対称ではありません(実装時に判明)。** 小さい方の実績で
> 判定しているので、**上振れ**なら大きい方でも上振れです(確定)。しかし
> **下振れ**は、小さい方が下回っただけでは大きい方について何も言えません——
> AAPLと逆に**特別損失**を計上した会社は、同じ差が逆向きに出ます。下振れは
> 点数を**引く**ので、そのまま扱うと「どちらの数字が比較可能か分からない」という
> こちらの都合が「会社の成績が悪かった」と読まれてしまいます。そこで、
> **下振れは大きい方の実績でも下回っている場合にのみ成立**とし、それ以外は
> 未検証(0点)に戻します(不変条件6)。
>
> なお、これは棄却済みの案「実績値の不一致しきい値(2%)を緩める」とは**別物**
> です。しきい値は1ミリも動いておらず、食い違いの検出も、発表文への
> エスカレーションも従来どおりです。変えたのは「検出したあと柱をどう扱うか」
> だけです。
>
...
配点は**上振れ時に加点(+5.0)、下振れ時に減点(-5.0)、不在は0**。
⚠️ **下振れの減点は2026-08-11にUser判断で追加しました**(それ以前は減点なし)。
理由: 会社が実際に弱い見通しを**出した**という事実を0点にすると、**何も言わなかった
会社と同じ点数**になり、両者の区別が消えていました。**不在が0点であることは変えて
いません** — 見通しを出していない/但し書き付きで比較を見送った/物差しが無い、の3つは
すべて0点です。不在を罰すると会社ではなくデータの穴を罰することになるためです。
「ガイダンス無し」という事実は審理に渡し、反証役がそれを攻撃するのは自由ですが、
**機械的な不利は与えません**。
...
**これは事前登録がなぜ必要かの、設計時より強い根拠**です——決算後にはその四半期の
予想はもうどこからも取れません。

**【2026-08-09 追記】Yahooの予想エンドポイントはもう読んでいません。** この1期ズレ
を `shift_after_print()` で読み替えていましたが、ガイダンスの比較対象は決算専門
サイトの要約文（会社のガイダンスが書かれているのと同じ文）から読むようになり、
この関数ごと削除しました。上の観測は事前登録の必要性の根拠としてのみ残します。
...
**運用上の含意(2026-08-13 更新、Set G・`docs/design/SET_H_G_DECISIONS.ja.md`)**:
上の「①」が事前登録を必要とした最大の理由(Yahoo Financeの予想取得エンドポイントが
決算後に1四半期分ズレた数字を返すバグ)は、2026-08-09にそのエンドポイントの
コード自体が削除されて解消済み。現在、決算後にコンセンサスを再構成する処理
(`reconstructed_consensus`)はEWの決算要約文からその場で翌四半期・通期の予想を
読み取るため、同じズレが起きない。`hawkeye consensus capture` を**決算シーズン中に
毎営業日回す運用はもう必須ではない** — 事前登録が無い銘柄・四半期は、走査
(`hawkeye scout`)が無言で `reconstructed_consensus` に切り替えて判定する
(既にテスト済みの経路)。事前登録の仕組み自体(コード・コマンド・DBのテーブル・
テスト)は削除せず、必要な時に使える機能として残してある。判断の根拠と可逆性は
`docs/design/SET_H_G_DECISIONS.ja.md` を参照。
...
| `earnings_prints`(決算実績) | 銘柄 × 会計四半期(有効な行は常に1つ) | 発表日、EPS実績、決算カレンダーが返した実績(重複行を含む)、売上実績、**会社の見通し(読み取れた全期。2026-08-19、T-020)**、汚染フラグ、**どのデータ提供元の数値か(`source`)**、**その行がまだ有効か(`status`)**、**発表直前に有効だったコンセンサス行のID** |
...
`earnings_prints` の各行に、**その数値がどのデータ提供元から来たか**(`source`)を
持たせます。

| `source` | 意味 | 追加コスト |
|---|---|---|
| `finnhub` | 決算カレンダーの応答に入っていた予想と実績の生値だけ | **ゼロ** |
| `yahoo` | 順位付けの前に第2ソースで実績を取り直した | Yahoo呼び出し1回 |

**この列は「どのデータ提供元か」だけを表し、審理の結論は決して書き込まれません。**
書き込めるようにした瞬間、「順位を付けた時点で何を知っていたか」が答えられなくなり、
事前登録の意味が消えます。

> **2026-08-07 変更**: もとは `depth`(記録の深さ)という4段階の列で、`calendar_only` /
> `verified` / `xbrl_validated` / `release_read` を持っていました。うち
> `xbrl_validated` はシステムの歴史上**一度も代入されたことがなく**、`release_read` は
> 発表文の読み取り撤去で**到達不能**になりました。残る2段は「どのデータ提供元か」と
> 同じ情報だったので、二重に持つのをやめて `source` に一本化しています。
> `yahoo` は暫定値で、EarningsWhispersを走査に配線した時点(タスク4)で `whispers` に
> 置き換わります。

**選別に上がった銘柄は、その後どこで落ちても決算カレンダー由来の行を残します**
...
肉付けの walk のあとに実行するので、既に行がある四半期は塗り替えません)。
...
**【2026-08-03(b) 追加】前回実行から日が空いたときだけ、当日発表分も取ります。**
対象期間は「翌営業日から2営業日」で、**当日は入りません**。これは通常は正しい
挙動です——当日の米国決算は日本時間のその晩に出るので、前日の実行が既に登録済み
だからです。しかし実行は手動なので、空いた日があると当日分は**恒久的に2回の実行の
間に落ちます**。期間の計算はマシンのローカル日付(JST)基準、カレンダーの日付は
米国市場日なので、2026-08-03の10:08に走らせた実行は米国08-04〜05を見ており、
米国08-03(日本時間その日の20時頃から発表)は捕まえられず、あとから取ることも
できません。そこで、**前回の事前登録から `consensus_capture_include_today_after_days`
(=2)日以上空いていれば、当日も対象に含めます**。安全性は仕組みで担保されています
——実績値が入っている行は元から除外されるので、既に発表された決算が「事前登録」
として紛れ込むことはありません。「前回いつ走ったか」は専用のテーブルを作らず、
**事前登録行そのものの最新取得時刻**から読みます(事後復元の行は数えません)。
1件も取得しなかった実行は時計を進めないため、判定は「空いた」側に倒れます
——取り逃したスナップショットは復元不能で、無駄な問い合わせは1回で済むためです。

**【2026-08-03(b) 追加】`earnings_prints` と `consensus_snapshots` に `ticker` 列**を
足しました。両テーブルの鍵はCIKで、SQLで直接覗いたときに人間が読めないためです。
**SQLiteの生成列(GENERATED ALWAYS AS … VIRTUAL)**なので、既存行の書き戻しは
一切発生せず(追記専用トリガーはUPDATEを拒否します)、payloadとずれることも
原理的にありません。読みやすさのためだけの列で、コードは従来どおりpayloadを見ます。

#### (E) 「そもそも建玉になり得ない銘柄」には予想を取りに行かない(2026-08-03b 実装)

コンセンサスの事前登録は、2営業日以内に決算予定の**全米国銘柄**が対象で、
2026-08-03の実測で855件・1件につきYahooへ1回の問い合わせでした。このうち
相当数は、決算がどれだけ良くても**建玉になり得ません**——株価が下限未満、
時価総額が下限未満、あるいは事前登録したストップで抜けられるだけの流動性が
無い銘柄です。毎日それらに問い合わせても得るものがありません。

**この機能を支配しているのは1つの非対称性です:取らなかったスナップショットは、
後から絶対に取れません。** 誤って除外した銘柄はコンセンサスの履歴を永久に失い、
誤って含めた銘柄はAPIコール1回を無駄にするだけです。そこで:

- **まだ誰も判定していない銘柄は「対象」**(未判定は「対象外」ではない)
- 除外するのは、**明示的で、かつ新しい**判定があるときだけ
- 判定は**失効する**(`config.stock_triage_ttl_days` = 90日)。3ドルの会社は
  来四半期に9ドルの会社になり得るためです

判定そのものはタダです。ファネルが既に実行した**入口ゲートの結果を読むだけ**で、
そのうち**会社の性質を表す3つ**(`min_price` / `min_market_cap` /
`min_avg_dollar_volume`)しか見ません。`catalyst_freshness`(決算からの経過日数)は
ハードゲートですが**その決算についての事実**であり、それで会社を除外すると
「見るのが遅れた」だけの優良企業を永久に落とします。未検証のゲートも判定に
使いません——無料枠のデータ欠損を永久除外に変換するのは、素通りの裏返しです
(不変条件6)。

判定は投影であり、**`hawkeye stocks rebuild` で台帳から再構築できます**
(落選記録に凍結されている入口ゲートの結果が事実で、マスタはその読み)。
再構築時の判定日は**その記録の日付**であって実行日ではありません。さもないと
再構築のたびに失効期限が延びます。

**実測(2026-08-03b時点)**: 入口ゲートまで到達したことのある銘柄は36件で、
うち**10件(28%)が構造的条件で落ちて**います(流動性9・時価総額4・株価2、重複あり)。
ただし現在マスタにある870件のうち判定を持つのは**2件だけ**で、いま除外される
銘柄は**0件**です。36件の多くは決算日が古く、マスタ(直近の事前登録で作られた行)に
存在しないためです。**この機能の効き目は、ファネルが回るほど育ちます**。
...
erDiagram
    ...
    STOCK {
        ...
        bool investigation_target "投影:未判定(NULL)は対象扱い。§6.1(E)"
        string investigation_reason "落とした入口ゲート名"
        datetime investigation_checked_at "判定日。一定期間で失効"
    }
    EARNINGS_PRINT {
        string stock_id FK
        string fiscal_quarter "2026-Q2"
        date report_date
        string source "finnhub か whispers。実績とコンセンサスは必ず同じ提供元"
        string status "active か superseded。有効なのは1四半期1行"
        datetime superseded_at "改訂で無効化された時刻"
        float eps_actual "この行が拠って立つEPS実績"
        float eps_actual_rows "カレンダーが返した実績。重複行は全て保持"
    ...
    CONSENSUS_SNAPSHOT {
        ...
        float eps_avg "分布を出すソースの平均"
        float eps_low
        float eps_high
        int analyst_count "3未満は未検証"
        float eps_calendar "決算カレンダーの点推定"
        float revenue_avg
        float revenue_calendar
        float next_quarter_eps_avg "ガイダンスの物差し(翌四半期)"
        float full_year_eps_avg "ガイダンスの物差し(通期)。中身の供給は未実装"
    }
```
...
- `docs/design/CAUSE_SOURCE_EXTRACTION.ja.md` **「決算理由の情報源と、その抜き出し
  工程の設計」**(2026-08-17) — 決算がなぜその数字になったかを会社の言葉から読み取る
  工程が実環境で30件中0件しか理由を取れなかった問題への対処(T-008)。読ませている
  EWサマリーに過去四半期の理由が構造的に含まれないことが原因で、決算プレスリリース
  全文を新しい情報源として足す。本文が1銘柄あたり約25,900文字と大きいため、理由の
  書かれたブロックをGeminiに**原文のまま抜き出させ**(要約させない)、抜き出された文が
  原文に存在するかをコード側で機械照合してから読み手に渡す。**機械照合は形式ではなく
  実際に効いている**: 13銘柄の実測で、明示的に禁止してもGeminiが1件、原文に無い文を
  流暢な英語で合成した。**未実装・User承認待ち**。
