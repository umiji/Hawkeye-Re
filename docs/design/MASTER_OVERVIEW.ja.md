...
- **中心仮説(＝目標を達成するための手段)**: 人間特有の「保有している銘柄への
  思い入れ」「都合の良い理由付け」を機構的に排除し、投資アイデアに徹底的な反証を
  仕掛けるプロセスを回せば、人間の投資チームより質の高い判断ができるはずだ、という
  仮説。**質の高い判断は手段であって、目的は上の年率50%である。**この仮説の検証は、
  目標に近づいているかを測るために行うのであって、検証それ自体が目的ではない。
- **記録の役割**: 何を根拠にどう判断し、それが実力によるものか運によるものかを
  厳密に記録するのは、**目標に届かなかったときに原因を特定して直すため**である。
  較正できているのに50%に届かないなら原因は「量」(試行回数・利益の伸ばし方・
  リスク量)であり、較正できていないならエッジが実在しない——記録が無ければこの
  2つを区別できず、どちらに手を打つべきか永久に分からない。記録は計測器であって
  成果物ではない。**利益を出せないシステムは、どれだけ記録が美しくても失敗である。**
...
### 原則5: 自己採点による複利的な学習ループ(改善速度を上げるための機構)

ここは「目標に到達するための改善速度」を決める部分です。ほとんどの個人投資家・小規模ファンドは、自分の
...
これにより、**「間違いを検出し、修正できる」という性質が、リターンを積み上げる
速度そのものになります**。1回の判断精度が同じでも、外した原因を特定して次に直せる
システムは回を重ねるごとに勝率と期待値が上がり、直せないシステムは同じ失敗を
繰り返して原資を削ります。著名なクオンツファンドが「シグナル単体の精度よりも
プロセスの自己修正能力が長期成績を決める」と言うのはこの意味であり、**自己修正能力
それ自体が目的なのではなく、それが最終的な運用成績に効くから重視する**という順序です。
...
`strategy/ROADMAP.md` およびこの後の「4. As-Is」章)。

ここで誤解してはいけないのは、**検証ゲートは目標を下げるための仕組みではない**
ということです。数十件〜100件程度の実績では年率と運を統計的に分離できないため、
年率そのものをゲートに使うと運の良い期間を通してしまう——だから途中の判定には
較正という早く読める代理指標を使う、というだけの話です。目標は年率50%のまま
動きません。50%未達で較正されたプロセスは、**「継続投資に値する」ではなく「継続
投資に値する、ただし差を埋める具体策とセットで」**が正しい読み方で、その差は
現状「量」の問題(試行回数・利益の伸ばし方・リスク量が未装着)だと既に自己監査で
特定済みです(`strategy/STRATEGY_BACKLOG.ja.md`)。運による50%達成は、再現しない
ので継続投資に値しません。
...
| `earnings_prints`(決算実績) | 銘柄 × 会計四半期(有効な行は常に1つ) | 発表日、EPS実績、決算カレンダーが返した実績(重複行を含む)、売上実績、ガイダンス、汚染フラグ、**どのデータ提供元の数値か(`source`)**、**その行がまだ有効か(`status`)**、**発表直前に有効だったコンセンサス行のID** |
