...
| 決算の3本柱判定(2026-08-02) | EPS・売上・ガイダンスをそれぞれ2ソースのコンセンサスと突き合わせ、**両ソースが揃って上振れと言った場合のみビート**とする。食い違い・アナリスト不足・分母過小はすべて未検証(0点) | `hawkeye/scout/quality.py`, `hawkeye/reports/quality_ja.py` |
...
いました。この教訓自体は生きています——**48時間の実績待ち(タスク6)でも、
保留中の決算を重複排除が弾かないようにする例外が同じ理由で必要**です。
...
| **決算数値のソース** | 判断の土台になる数値は、企業の1次情報と一致している | **発見=Finnhub / EPS数値=Yahoo に分離済み(2026-08-02)**。ランキングの**前**に第2ソースで取り直し、公表サプライズ率をそのまま採用(再計算しない)。照合上限60件/回、`source` とカレンダー側の読みを両方記録。**一次資料(EDGAR XBRL)による検算は2026-08-07に廃止** | **2ソース一致を要件とする方式を実装済み(2026-08-02)**。実績値が2%超かつ$0.01超で食い違えば `actual_disputed` を立て、**小さい方の実績×大きい方の予想**で評価する(どちらの数字でも成立する読みなので上振れは成立する。ただし基準は未決着で、**この系に決着させる手段はもう無い**)。コンセンサスが5%超食い違えば `consensus_disputed` を立てて**保守的な読みで順位付け**。Finnhubが同じ決算に矛盾する実績を返した場合、その実績は**使用不能**。過去に記録済みの候補には遡って適用できない | 実績値の食い違いは25%、うち選別の合否が入れ替わるのは19%。**入れ替わりの2/3はコンセンサス側が原因**で、実績値の精度を上げても解けない。→ 1銘柄1ソース(EW移行)にすれば食い違い自体が発生しない。実装はタスク4・7.5(`docs/backlog/PIPELINE_BUILD_TASKS.ja.md`) |
| **「良い決算」の定義** | 決算をきっかけに買う以上、その決算が良いことが審査の柱になる | **EPS・売上・ガイダンスの3本柱で判定済み(2026-08-02実装、`hawkeye/scout/quality.py`)**。ビートは**2ソースが揃って上振れと言った場合のみ**。ガイダンスは**ゲートにせず**、上振れ時のみ小さな加点、**不在は0点で減点なし**。EPSレンジを出さない会社(AMZN等)は**売上レンジの中央値**で判定 | ガイダンスの供給が**未実装**。コンセンサス行に**翌四半期と通期の両方の物差しの欄は用意済み(2026-08-07)**だが、中身を埋める処理が無い。手で与える経路(`scripts/dry_run_earnings_quality.py --guidance`)のみ。公表Non-GAAP EPSを読む経路は**2026-08-07に廃止**(発表文の読み取り自体を撤去したため) | ガイダンスには無料の構造化ソースが存在せず、LLM抽出が唯一の道。全銘柄に読ませるとトークン費用が見合わないため、**上位15枠を確定させた後にだけ読む**設計。実装はタスク8.7(`docs/backlog/PIPELINE_BUILD_TASKS.ja.md`)。実測では47銘柄中10社が通期のみで見通しを出しており、通期の物差しが無いとこの10社は「ガイダンス開示なし」と誤読される |
...
> ⚠️ **【2026-08-07 部分的に撤回】この節は当時の設計判断の記録であり、現在の実装
> ではありません。** 以下は**もう存在しない**ので、読むときに読み替えてください。
> なお2ソース照合そのものは、EW移行(タスク4)が完了するまでは現役です。
>
> | この節の記述 | 現在 |
> | --- | --- |
...
erDiagram
    ...
    EARNINGS_PRINT {
        ...
        date report_date
        string source "finnhub か yahoo。EW配線後は finnhub か whispers"
